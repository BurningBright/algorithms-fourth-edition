...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    *************************************************************************/
     ...
     * @param type lines between center - begin - end
     * @throws IllegalArgumentException if the radius of the circle is negative
     */
    public static void arc(double x, double y, double r, double angle1, double angle2, int type) {
        ...
        double hs = factorY(2*r);
        if (ws <= 1 && hs <= 1) pixel(x, y);
        else offscreen.draw(new Arc2D.Double(xs - ws/2, ys - hs/2, ws, hs, angle1, angle2 - angle1, type));
        draw();
    }
    
    /**
     * fill a arc from radius r, centered on (x, y), from angle1 to angle2
     * the type show the line between center - begin - end relation
     * @param x the x-coordinate of the center of the circle
     * @param y the y-coordinate of the center of the circle
     * @param r the radius of the circle
     * @param angle1 the starting angle. 0 would mean an arc beginning at 3 o'clock.
     ...
     * @param type lines between center - begin - end
     */
    public static void filledArc(double x, double y, double r, double angle1, double angle2, int type) {
        if (r < 0) throw new IllegalArgumentException("arc radius must be nonnegative");
        while (angle2 < angle1) angle2 += 360;
        double xs = scaleX(x);
        double ys = scaleY(y);
        ...
        double hs = factorY(2*r);
        if (ws <= 1 && hs <= 1) pixel(x, y);
        else offscreen.fill(new Arc2D.Double(xs - ws/2, ys - hs/2, ws, hs, angle1, angle2 - angle1, type));
        draw();
    }
    ...
    *************************************************************************/
    ...
    /**
     * used to draw curve line
     * @param xa the x-coordinate data
     * @param ya the y-coordinate data
     */
    public static void curves(int[] xa, int[] ya) {
        int[] x, y;
        double[] a, b, c;
        ...
        if (n > 1) {
            ...
            for (i = 0; i < n - 1; i++) {
                ...
                while (t < tt[i + 1]) {
                    ...
                    px2 = (int)cx;
                    py2 = (int)cy;
//                    offscreen.drawLine(px1, py1, px2, py2);
                    offscreen.draw(new Line2D.Double(px1, py1, px2, py2));
                    px1 = px2;
                    py1 = py2;
                }
            }
    ...
    public static void curves(double[] xa, double[] ya) {
        double[] x, y;
        double[] a, b, c;
        double[] px, py, qx, qy, tt;
        double[] dx, dy;
        ...
        bx4 = 0;
        by3 = 0;
        es = 0.03 * Math.sqrt((xmax - xmin) * (xmax - xmin) + (ymax - ymin) * (ymax - ymin))
                / Math.sqrt((DEFAULT_XMAX - DEFAULT_XMIN) * (DEFAULT_XMAX - DEFAULT_XMIN)
                        + (DEFAULT_YMAX - DEFAULT_YMIN) * (DEFAULT_YMAX - DEFAULT_YMIN));
        px[0] = 1;
        py[0] = 1;
        px[n - 1] = 1;
        py[n - 1] = 1;
