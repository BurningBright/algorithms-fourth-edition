...
public class SETdouble {
    ...
    ***************************************************************************/
    ...
    private Node delete(Node h, double key) { 
        ...
        if (sub(key, h.key) < 0)  {
            ...
            if (isRed(h.left))
                h = rotateRight(h);
            if (sub(key, h.key) == 0 && (h.right == null))
                return null;
            if (!isRed(h.right) && !isRed(h.right.left))
                h = moveRedRight(h);
            if (sub(key, h.key) == 0) {
    ...
    ***************************************************************************/
    ...
    private void keys(Node x, Queue<Double> queue, double lo, double hi) { 
        if (x == null) return; 
        double cmplo = sub(lo, x.key); 
        double cmphi = sub(hi, x.key); 
    ...
    ***************************************************************************/
    ...
    private boolean isBST(Node x, Double min, Double max) {
        ...
        if (min != null && sub(x.key, min) <= 0) return false;
        if (max != null && sub(x.key, max) >= 0) return false;
